...
export default {
  ...
  watch: {
    aktuellt_soekord_input(newS, oldS) {
      console.log(newS);
    },
  },
    ...
    <header class="pb-3 mb-4 border-bottom">
      <form class="form-control">
        <span class="fs-4">
          ...
          />
          <input type="button" v-on:click="onClick" value="Sök" />
        ...
        >
          <p>
            <RouterLink :to="'/laenkar/' + artikeltitel1"
              >Se alla länkar från <i>{{ artikeltitel1 }}</i></RouterLink
            >
          </p>
    ...
    >
      <div class="col-md-6" v-if="laenk_arr.length">
        <div class="h-100 p-5 text-white bg-dark rounded-3">
          ...
          <p id="hela_artikeln" v-html="hela_artikelns_text"></p>
        </div>
      </div>
      <div class="col-md-6">
        <div class="h-100 p-5 bg-light border rounded-3">
          <h2 v-html="titel_spraak2"></h2>
          <hr />
          <p v-html="artikeln_spraak2"></p>
        </div>
      </div>
    </div>
